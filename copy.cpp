...
struct TRAIN {
    ...
    struct TRAIN* prev;
    TRAIN() {}
    TRAIN(std::string dest, std::string time, std::string train_num) : dest(dest), time(time), train_num(train_num), next(nullptr), prev(nullptr) {}
};
...
public:
    ...
    /*void swapItems(TRAIN* a, TRAIN* b) {
        TRAIN* tmp1;
        TRAIN* tmp2;
        if (a->next == b) {
            tmp1 = a->prev;
        ...
        if (a->prev == nullptr) first = a;
        if (a->next == nullptr) last = a;
        if (b->prev == nullptr) first = b;
        if (b->next == nullptr) last = b;
    } */

    TRAIN * swapItems(TRAIN *lst1, TRAIN *lst2, TRAIN *head)
    {
        // Возвращает новый корень списка
        TRAIN *prev1, *prev2, *next1, *next2;
        ...
        for (i = first; i->next != nullptr; i = i->next) {
            for (j = i->next; j != nullptr; j = j -> next) {
                if ((k == 1) && (i->dest > j->dest) || (k == 2) && (i->time > j->time) || (k == 3) && (i->train_num > j->train_num)) {
                    i_tmp = i;
                    j_tmp = j;
                    this->first = swapItems(i_tmp, j_tmp, this->first);
                }
            }
        }
    }
...
int main() {
    ...
    do {
        ...
        switch (n) {
            ...
            case 6: {
                //sort_list(l);
                //std::cout << l->first->dest << l->last->dest << std::endl;
                //l->swapItems(l->first, l->last, l->first);
                //std::cout << l->first->dest << l->last->dest << std::endl;
                l->bubbleSort(1);
                break;
            }
